...
subtitle: Written by Tom Beesley 
order: 8
editor_options: 
...
*Please note: Lecture recordings do not seem to show properly using Safari browser (Mac). Please try Chrome and if problem persists, report on the Moodle Discussion Forum.*
...
-   Download the [csv file for Week 7](data/Week_7/salary_data_wk7_2025.csv) and upload it into this new folder in RStudio Server.
...
9.  We may also want to filter out the very high estimates. Let's keep all those values less than (\<) £100,000 (again, this is arbitrary, but we are just doing this so we can learn how to use the `filter()` command). You can copy and edit the code above to run another filter command. Remember, you'll need to think about which data is going IN to the calculation, and what you will assign the result to.

10. When you've done both filter commands, the (new) object should have **69 rows**. We can now plot the data again as a histogram. To do this, **copy the earlier code for the histogram** and edit it as necessary (if you're using a new object for the filtered data, you'll need to edit the code to reflect that).
...
We now want to know if the salary estimates are different to the actual average salary in the UK. According to [ONS](https://www.ons.gov.uk/employmentandlabourmarket/peopleinwork/earningsandworkinghours/bulletins/annualsurveyofhoursandearnings/2024) this was £37,430 in 2024. Our hypothesis might be that people are inaccurate - they overestimate or underestimate the average UK salary. Let's test that.
...
t.test(d_f$salary_estimate, mu = 37430)
...
#### Practice this process on the "Climate change" column

We've also included a column labelled `human_CC`. For this survey question we asked:

::: {.callout-tip icon="false" title="Human made climate change"}
How much do you think human activity contributes to climate change, as a percentage of overall climate change? Please enter a value between 0 (humans do not contribute to climate change at all) and 100 (human contribute entirely to climate change).
:::

Feel free to practice what you've learnt so far in this week's task, but with this new column (e.g., calculate a mean, draw a plot of the data). It's a little harder to be sure exactly what % of climate change is actually driven by human activity, but scientists believe it is likely to be very close to 100% (see [this rather effective infographic](https://xkcd.com/1732/)). For your statistical test, you could ask whether our sample believes human climate change is less than 100%. Discuss on your table whether you think it is appropriate to filter any of the data out before you run this test.

You will also see in the data for this week, that we have a column that is labelled `attention_check` . This column represents answers to a survey question where we said "Please select strongly agree to show you are paying attention". You may want to add in code to your analysis to use a filter command to remove those participants who failed the attention check. Run your analysis again to check if anything has changed (i.e., the graphs and stats).

### Task 3 - Test your understanding of filtering
...
2.  Just those people who come from the South East (10 rows)

3.  Those people who come from North West and are the oldest child within their siblings (17 rows)

4.  Those people who failed the attention check (6 rows)

5.  Those people passed the attention check and are the youngest child (14 rows)

6.  Those people who are NOT from the North West (hint: you'll need to use `!=`) (28 rows)

7.  Those people who are from the South East or (`|`) the South West (13 rows)
