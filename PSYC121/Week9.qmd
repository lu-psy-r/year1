...
[Chapter 14 of Howell](https://onesearch.lancaster-university.uk/permalink/44LAN_INST/1438dph/alma9930907993701221)
...
write_csv(data_wk9, file = "data/Week_9/data_wk9.csv")
...
# 
#  write_csv(d_stroop, file = "data/Week_9/data_stroop_wk9_2025.csv")

d_stroop <- read_csv("data/Week_9/data_stroop_wk9_2025.csv")
...
Create a folder for Week 9, download the [Week_9 csv file](data/Week_9/data_wk9.csv) and upload it into this new folder in RStudio Server.
...
7. You should have **removed 1 row of data** to make an object with 59 rows. Make a note of this in your R markdown document. 
...
3. In order to do a t-test (and variance test) we will need to split the data into two conditions. Complete the following filter code to first get only the participants from the North, then all the participants from the South:
...
north_condition <- data_wk9_z %>% filter(home_location_in_UK == "North")
south_condition <- data_wk9_z %>%filter(home_location_in_UK == "South")

# check if variances are unequal (p < .05) - Q3
var.test(x = north_condition$pop_est_immigrants, y = south_condition$pop_est_immigrants, data = data_wk9_f)
...
# check if variances are unequal (p < .05) - Q3
var.test(x = north_condition$pop_est_age, y = south_condition$pop_est_age, data = data_wk9_f)

# run unrelated samples t-test
t.test(x = north_condition$pop_est_age, y = south_condition$pop_est_age, 
       paired = FALSE,
       var.equal = TRUE) # you'll need to set this to TRUE or FALSE depending on what you found in var.test
...
2.  Import the  [new stroop data csv file](data/Week_9/data_stroop_wk9_2025.csv). We have reduced the data down to just the *compatible* and *incompatible* conditions.
...
pwr.t.test(d = 1.42, n = 20, type = "paired") #Q4

pwr.t.test(d = 1.42, power = .8, type = "paired") #Q4
...
You can access a set of quiz questions related to this week [here.](https://ma-rconnect.lancs.ac.uk/PSYC121_Week_9_quiz/){target="_blank"}
